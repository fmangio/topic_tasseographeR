...
* **Topic–word distribution (β)** (`td_beta`):
  A CSV file with columns `topic`, `term`, and `beta`, representing the probability or weight of each term within each topic.

* **Document–topic distribution (θ)** (`ds`, Topic Function module only):
  A CSV file containing the original document text and one column per topic (`Topic_k`) with document‑level topic proportions.

### Dictionary selection

Users can:

* upload a validated dictionary as a CSV file, or
